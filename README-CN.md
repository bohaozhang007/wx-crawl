...
│   ├── hermes/                  Hermes no-agent 定时任务启动脚本
...
默认情况下，命令只向标准输出打印一份简短的 JSON 摘要；各公众号明细和工具日志
仍会保存在本次运行记录中。只有在人工排查问题、需要观察终端详细进度时，才加上
`--verbose`：
...
如果希望采用 no-agent 方式运行，并在结束后把简短结果直接发送到已配置的钉钉群，执行：
...
定时爬取使用 Hermes 原生的纯脚本 `no_agent` 模式，在本地时间每天 12:05 和
20:00 运行。Hermes 会直接执行 `~/.hermes/scripts/wx_crawl_no_agent.sh`，不会创建
Agent 会话，也不会调用大模型。认证检查、二维码投递、五分钟扫码等待、正式爬取、
结果记录和服务关闭均由 Python 程序完成；脚本输出的简短报告由调度器直接投递到钉钉。
...
两个爬取任务都应显示 `Mode: no-agent`。不要再创建功能相同的 Agent 模式定时任务，
否则会造成重复爬取，并重新引入模型调用和 Agent 轮询。

22:00 的后续任务只执行一次确定性的待处理批次命令，内部完成 V2 打标、筛选、数据库
入库、清理和 AI 表同步。如果 20:00 的爬虫仍在运行，该命令会先等待爬虫文件锁释放，
再枚举待处理批次，从而纳入刚刚完整落盘的运行记录，不需要 Agent 轮询。
如果个别文章打标失败，Python 会再尝试一次，并自动跳过已有的有效 V2 标签，只重新请求
尚未解决的文章。若重试后仍失败，该批次会保留到下一次运行继续处理，但不会阻断后续独立
批次的筛选和入库。

后续处理程序会在每个高层阶段结束后向钉钉发送一次简短汇总，三个阶段分别是：打标、
筛选与报告生成、数据库入库与 AI 表同步。程序不会按文章、批次或重试过程发送进度消息。
这些通知由 Python 直接调用 Webhook 发送，不会写入 Hermes Agent 的对话历史，也不会占用
模型上下文。如果通知投递失败，错误只会写入本次流水线的执行明细，不会中断文章处理。
独立运行的 no-agent 爬取任务仍会在爬取结束后发送自己的一次完成报告。

在新机器上安装时，需要把 `install/hermes/wx_crawl_no_agent.sh` 复制到
`~/.hermes/scripts/` 并赋予执行权限，再使用
`--script wx_crawl_no_agent.sh --no-agent` 创建或转换 12:05、20:00 两个任务。
同时应将 Hermes 的 `cron.script_timeout_seconds` 设置为至少 `21600`，避免持续数小时的
爬取被默认的一小时脚本超时提前终止。

### 3.6 通过模型 API 为已下载文章打标

这个命令负责打标，并在同一次模型响应中生成每篇文章的摘要；它不会启动爬取，
也不会执行筛选、生成最终筛选报告或写入 SQLite。
...
才会原子写入 `label.json`。同一次 API 响应还会为每篇 KEEP、DROP 或 REVIEW 文章
生成 1～3 句话的客观 `summary`，筛选后不再由 Agent 单独读取正文并调用模型生成摘要。
已经具有摘要的有效 v2 标签会自动跳过；缺少摘要的 v2 标签在再次处理时会补齐。
v1 标签不再支持，应直接删除而不是升级。无效标签只有在新结果通过校验后才会被替换。
如果确实需要重新判断已经具有完整 v2 标签的文章，再使用 `--replace`。
默认标准输出只包含固定的汇总字段和 `details_file` 路径；逐篇处理结果保存在本次
运行目录的 `labeling_result.json` 中。只有人工需要查看逐篇进度和完整 JSON 时才加
`--verbose`。这样 Hermes 在正常串联多个阶段时，不会把文章级明细不断累积进主会话
上下文。

### 注意事项

- 第一次需要认证或凭据失效时，程序会把二维码直接发送到已配置的钉钉群，并在本次运行目录的 `tools-log` 下临时保存二维码文件。请在五分钟内用微信扫码。如果上游状态接口最初误把旧凭据判为有效，但真实的公众号或历史文章请求拒绝了该凭据，程序也会识别这个认证错误，强制发出新二维码，并在登录成功后自动重试一次中断的操作。认证完成后二维码会被删除，凭据则保存在本地供后续运行复用。
...
两层标签、模型同次生成的摘要和正文文本；未入选文章仍会先保留，确认数据库导入成功
后才可以清理。
...
- 如果凭据已经过期，或真实请求返回认证错误，请保持本次任务运行并扫描钉钉收到的新二维码；登录成功后程序会自动重试。
